...
def get_data(french_training_file, english_training_file, french_test_file, english_test_file):
    ...
    # 1) Read French and English Data for training and testing (see read_data)
    french_training = np.array(read_data(french_training_file))
    english_training = np.array(read_data(english_training_file))
    french_test = np.array(read_data(french_test_file))
    english_test = np.array(read_data(english_test_file))

    # french_training = french_training[:, :FRENCH_WINDOW_SIZE]
    # english_training = english_training[:, :ENGLISH_WINDOW_SIZE]
    # french_test = french_test[:, :FRENCH_WINDOW_SIZE]
    # english_test = 	english_test[:, :ENGLISH_WINDOW_SIZE]

    # 2) Pad training data (see pad_corpus)
    french_training, english_training = pad_corpus(
        french_training, english_training)

    # 3) Pad testing data (see pad_corpus)
    french_test, english_test = pad_corpus(french_test, english_test)

    # 4) Build vocab for french (see build_vocab)
    # build_vocab(np.append(french_training, french_test))
    french_vocab, _ = build_vocab(french_training)

    # 5) Build vocab for english (see build_vocab)
    # build_vocab(np.append(english_training, english_test))
    english_vocab, english_padding_id = build_vocab(english_training)

    # 6) Convert training and testing french sentences to list of IDS (see convert_to_id)
    french_training = convert_to_id(french_vocab, french_training)
    french_test = convert_to_id(french_vocab, french_test)

    # 7) Convert training and testing english sentences to list of IDS (see convert_to_id)
    english_training = convert_to_id(english_vocab, english_training)
    english_test = convert_to_id(english_vocab, english_test)
# 	(2-d list or array with english training sentences in vectorized/id form [num_sentences x 15]),
# 	(2-d list or array with english test sentences in vectorized/id form [num_sentences x 15]),
# 	(2-d list or array with french training sentences in vectorized/id form [num_sentences x 14]),
# 	(2-d list or array with french test sentences in vectorized/id form [num_sentences x 14]),
...
# 	french vocab (Dict containing word->index mapping),
# 	english padding ID (the ID used for *PAD* in the English vocab. This will be used for masking loss)
    return english_training, english_test, french_training, french_test, english_vocab, french_vocab, english_padding_id
